...
(function () {
  ...
  function xToSvg(xYd) {
    return xYd * PX_PER_YD;
  }
  function yToSvg(yYd) {
    return (FIELD_HEIGHT_YD - yYd) * PX_PER_YD;
  }
  ...
  function metaFor(label) {
    ...
    return (
      POSITION_META[label] ||
      POSITION_META[label[0]] || { color: "#6b7280", shape: "circle" }
    );
  }
  ...
  function createBlankPlay(name = "New Play") {
    ...
    return {
      ...
      players: makeDefaultPlayers(5, "base-right", losYards),
      annotations: [],
    };
  }
  ...
  function getFormationPositions(teamSize, preset, los) {
    ...
    const map = {
      "base-right": {
        ...
        5: [
          { label: "Q", x: cx, y: backfield },
          { label: "C", x: cx, y: onLOS },
          { label: "X", x: cx - 10, y: onLOS },
          { label: "Z", x: cx + 4, y: onLOS - 0.3 },
          { label: "Y", x: cx + 10, y: onLOS },
        ],
        6: [
          { label: "Q", x: cx, y: backfield },
          { label: "C", x: cx, y: onLOS },
    ...
    // Bunch: receivers clustered tight around C; left-to-right order X-C-Z-Y.
    // X stays outermost-left and Y outermost-right; extra receivers fill the
    // inner slots, alternating left then right (H, A, B).
    function bunchN(n) {
      const wrCount = Math.max(0, n - 2);
      const row = ["C"];
      if (wrCount >= 1) row.unshift("X");
      if (wrCount >= 2) row.push("Y");
      if (wrCount >= 3) row.splice(row.indexOf("Y"), 0, "Z");
      const extras = ["H", "A", "B"];
      for (let placed = 0; placed < wrCount - 3 && placed < extras.length; placed++) {
        if (placed % 2 === 0) row.splice(1, 0, extras[placed]);
        else row.splice(row.indexOf("Y"), 0, extras[placed]);
      }

      const spacing = 2;
      const cIdx = row.indexOf("C");
      const arr = [{ label: "Q", x: cx, y: backfield }];
      for (let i = 0; i < row.length; i++) {
        const offset = (i - cIdx) * spacing;
        const isC = row[i] === "C";
        arr.push({
          label: row[i],
          x: cx + offset,
          y: isC ? onLOS : onLOS - 0.2,
        });
      }
      return arr;
    }
  ...
  function validatePlaybook(obj) {
    ...
    if (obj.schemaVersion !== SCHEMA_VERSION)
      return `Unsupported schema version: ${obj.schemaVersion}`;
    if (!Array.isArray(obj.plays)) return "Missing plays array";
    for (const play of obj.plays) {
      if (!play.id || !Array.isArray(play.players))
        return `Invalid play "${play.name || "?"}"`;
      for (const pl of play.players) {
        if (!pl.id || !pl.start || typeof pl.start.x !== "number")
          return `Invalid player in "${play.name}"`;
      }
    }
    return null;
  }
  ...
  function renderField() {
    ...
    // 5-yard reference lines. Skip the one at the LOS — we draw a thicker
    // line there separately so coaches can spot the snap point at a glance.
    for (let y = 5; y < FIELD_HEIGHT_YD; y += 5) {
      if (y === play.losYards) continue;
    ...
    // The line of scrimmage — same colour as the other yard lines but
    // appreciably thicker so it reads as the dominant horizontal reference.
    fieldLayer.appendChild(
      el("line", {
        class: "los-line",
        x1: 0,
        y1: yToSvg(play.losYards),
        x2: VIEW_W,
        y2: yToSvg(play.losYards),
      }),
    );
  }
  ...
  function buildPath(anchors, style) {
    ...
    if (style === "zigzag") {
      ...
      for (let i = 0; i < anchors.length - 1; i++) {
        ...
        const oscCount = Math.max(
          2,
          Math.round((len / PX_PER_YD) * oscPerYard),
        );
        const ux = dx / len,
          uy = dy / len;
        const px = -uy,
  ...
  function getRouteAnchorsAbs(player) {
    ...
    const rest = player.route.anchors.map((a) => ({
      x: xToSvg(a.x),
      y: yToSvg(a.y),
    }));
    return [start, ...rest];
  ...
  function renderAnnotations() {
    ...
    if (ballStart) {
      const g = el("g", {
        transform: `translate(${xToSvg(ballStart.x)}, ${yToSvg(ballStart.y)})`,
        id: "ballStartMarker",
      });
      g.appendChild(
        el("ellipse", {
  ...
  function renderRosterList() {
    ...
    for (const p of play.players) {
      const li = el("li", {
        class:
          "roster-item" +
          (selection?.kind === "player" && selection.id === p.id
            ? " selected"
            : ""),
        "data-id": p.id,
        onclick: () => {
  ...
  function renderPlayMeta() {
    ...
    document.getElementById("includeOnWristband").checked =
      play.includeOnWristband !== false;
    document.getElementById("playbookName").value = playbook.name;
    document.getElementById("teamSize").value = String(
      playbook.settings.playersPerSide,
    );
    document.getElementById("showFieldLines").checked =
      playbook.settings.showFieldLines;
  ...
  function findClickedRoute(evt) {
    if (evt.target.classList.contains("route-anchor")) {
      return {
        player: findPlayer(evt.target.dataset.playerId),
        anchorIndex: parseInt(evt.target.dataset.anchor, 10),
      };
    }
  ...
  function onSvgPointerDown(evt) {
    ...
    if (routeHit && routeHit.player) {
      selectRoute(
        routeHit.player.id,
        routeHit.anchorIndex >= 0 ? routeHit.anchorIndex : undefined,
      );
  ...
  function startRoute(playerId) {
    ...
    toast(
      "Click to add anchors. Enter or double-click to finish. Esc to cancel.",
    );
    renderRoutePreview();
  }
  ...
  function renderRoutePreview() {
    ...
    const anchorsAbs = drawingRoute.anchors.map((a) => ({
      x: xToSvg(a.x),
      y: yToSvg(a.y),
    }));
    ...
    if (previewPath) {
      previewLayer.appendChild(
        el("path", {
          class:
            "route-path" +
            (toolDefaults.routeStyle === "dotted" ? " dotted" : ""),
          d: previewPath,
          stroke: color,
          fill: "none",
        }),
    ...
    if (drawingRoute.cursor && all.length > 0) {
      ...
      const cursorAbs = {
        x: xToSvg(drawingRoute.cursor.x),
        y: yToSvg(drawingRoute.cursor.y),
      };
      previewLayer.appendChild(
        el("line", {
          class: "route-preview",
  ...
  document
    .getElementById("popClose")
    .addEventListener("click", closePlayerPopover);
  ...
  function deletePlay() {
    if (playbook.plays.length <= 1) {
      toast("Cannot delete the last play", true);
      return;
    }
  ...
  function interpolateRoute(player, t) {
    if (!player.route || player.route.anchors.length === 0)
      return { x: player.start.x, y: player.start.y };
    const pts = [player.start, ...player.route.anchors];
    let totalLen = 0;
    const segLens = [];
    for (let i = 1; i < pts.length; i++) {
  ...
  function animationStep() {
    if (!animation.playing) return;
    const elapsed = animation.paused
      ? animation.pausedAt - animation.startTime
    ...
    document.getElementById("timeReadout").textContent =
      `${((t * dur) / 1000).toFixed(1)}s / ${(dur / 1000).toFixed(1)}s`;
  ...
  function renderAnimationFrame(t) {
    ...
    if (ballMarker && ballCarrier) {
      ballMarker.setAttribute(
        "transform",
        `translate(${xToSvg(ballCarrier.x)}, ${yToSvg(ballCarrier.y)})`,
      );
    }
  }
  ...
  function savePlaybookToFile() {
    ...
    a.href = url;
    a.download =
      (playbook.name || "playbook").replace(/[^a-z0-9_\-]+/gi, "_") + ".json";
  ...
  function exportSvg() {
    ...
    a.href = url;
    a.download =
      (currentPlay().name || "play").replace(/[^a-z0-9_\-]+/gi, "_") + ".svg";
    a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
  ...
  function exportPng() {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((b) => {
        ...
        a.href = url2;
        a.download =
          (currentPlay().name || "play").replace(/[^a-z0-9_\-]+/gi, "_") +
          ".png";
        a.click();
  ...
  async function exportPdf(options) {
    if (!window.jspdf) {
      toast(
        "jsPDF not loaded — load the page with internet first or run a local server",
        true,
      );
      return;
    }
    ...
    if (!options.singlePlay && plays.length > 1) {
      ...
      doc.text(`${plays.length} plays`, pageW / 2, pageH / 3 + 0.7, {
        align: "center",
      });
      ...
      for (const [cat, items] of Object.entries(byCat)) {
        ...
        for (const { play, num } of items) {
          ...
          doc.text(
            `  ${num}. ${play.name}${play.playCallCode ? ` (${play.playCallCode})` : ""}`,
            0.5,
            y,
          );
          y += 0.22;
    ...
    if (options.includeRoster && playbook.roster?.length) {
      doc.addPage();
      doc.setFontSize(18);
      doc.text("Roster", 0.5, 0.7);
      doc.setFontSize(11);
      let y = 1.1;
      for (const r of playbook.roster) {
        doc.text(
          `${r.jerseyNumber || "--"}  ${r.name || ""}  ${r.defaultPositionLabel || ""}`,
          0.5,
          y,
        );
        y += 0.22;
    ...
    for (let n = 1; n <= pageCount; n++) {
      ...
      doc.text(
        `${playbook.name}  ·  Page ${n} of ${pageCount}`,
        pageW / 2,
        pageH - 0.25,
        { align: "center" },
      );
    ...
    doc.save(
      (playbook.name || "playbook").replace(/[^a-z0-9_\-]+/gi, "_") + ".pdf",
    );
    toast("PDF downloaded");
  }

  async function renderPlayToPdf(
    doc,
    play,
    num,
    pageW,
    ...
    if (window.svg2pdf) {
      try {
        await window.svg2pdf.svg2pdf(cloned, doc, {
          x: dx,
          y: dy,
          width: dw,
          height: dh,
  ...
  async function exportWristband(options) {
    ...
    const cardsPerCol = Math.max(
      1,
      Math.floor((pageH - margin * 2 + gap) / (sz.h + gap)),
    );
    ...
    doc.save(
      (playbook.name || "playbook").replace(/[^a-z0-9_\-]+/gi, "_") +
        "_wristband.pdf",
    );
  ...
  document
    ...
    .addEventListener("click", savePlaybookToFile);
  ...
  // Wristband dialog
  document.getElementById("wbCancel").addEventListener("click", () => {
  ...
  document
    .getElementById("intendedReceiver")
  ...
  document.getElementById("scrubber").addEventListener("input", (e) => {
    ...
    renderAnimationFrame(t);
    document.getElementById("timeReadout").textContent =
      `${((t * animation.durationMs) / 1000).toFixed(1)}s / ${(animation.durationMs / 1000).toFixed(1)}s`;
  });
  ...
  document
    .getElementById("includeOnWristband")
    .addEventListener("change", (e) => {
      snapshot();
  ...
  document
    .getElementById("duplicatePlayBtn")
    .addEventListener("click", duplicatePlay);
  document.getElementById("flipPlayBtn").addEventListener("click", flipPlay);
  document
    .getElementById("deletePlayBtn")
    .addEventListener("click", deletePlay);
